...
export class AnkiConnectService {
  ...
  async addNotesToAnki(cards: AnkiCard[], deckName: string, noteType: string) {
    ...
    for (const noteType of uniqueNoteTypes) {
      if (!this.noteTypeFields[noteType]) {
        const fields = await this.invokeAnkiConnect("modelFieldNames", { modelName: noteType });
        console.log(`笔记类型 ${noteType} 的字段名称:`, fields);
